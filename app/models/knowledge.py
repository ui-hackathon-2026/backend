...
class KnowledgeChunk(Base):
    ...
    inci_name: Mapped[str | None] = mapped_column(String(255), index=True, nullable=True)
    synonyms: Mapped[list] = mapped_column(JSON, default=list)
    cas_number: Mapped[str | None] = mapped_column(String(64), nullable=True)
    acd_number: Mapped[str | None] = mapped_column(String(32), nullable=True)
    max_concentration_pct: Mapped[float | None] = mapped_column(nullable=True)
    concentration_note_raw: Mapped[str | None] = mapped_column(Text, nullable=True)
